document.addEventListener('DOMContentLoaded', function () {
  ...
  rankingBody.innerHTML = '<tr><td colspan="3" class="text-center">Cargando ranking...</td></tr>';

  // Hacemos la petición al endpoint CORRECTO de la API
  fetch('api/ranking') // <-- CORRECCIÓN AQUÍ
    .then(response => {
      if (!response.ok) {
    ...
    .then(data => {
      ...
      if (data.success && data.ranking.length > 0) {
        ...
        rankingBody.innerHTML = '<tr><td colspan="3" class="text-center">No hay datos de ranking disponibles.</td></tr>';
      }
    })
    .catch(error => {
      console.error('Error al obtener el ranking:', error);
      rankingBody.innerHTML = '<tr><td colspan="3" class="text-center">No se pudo cargar el ranking. Inténtelo más tarde.</td></tr>';
    });
});