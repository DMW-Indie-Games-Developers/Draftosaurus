...
async function autoSave() {
  ...
  try {
    await fetch(apiUrl('/api/tablero/guardarEstadoPartida'), {
      method : 'POST',
...
async function cargarPartidaYRestaurar(idPartida) {
  const r = await fetch(apiUrl(`/api/tablero/cargarPartida?id=${idPartida}`), {
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' }
  });
  ...
  async function mostrarResultadosFinal() {
    try {
      ...
      if (ID_PARTIDA) {
        ...
        await fetch(apiUrl('/api/tablero/finalizarPartida'), {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
          ...
          return fetch(apiUrl('/api/tablero/eliminarPartida'), {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
  ...
  recintos.forEach(recinto => {
    ...
    recinto.addEventListener('drop', async e => {
      ...
      if (puedeColocarDino(recinto, tipoRecinto, especieDino)) {
        ...
        try {
          const validacion = await fetch(apiUrl('/api/tablero/validarJugada'), {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
