document.addEventListener('DOMContentLoaded', function () {
  ...
  fetch(apiUrl('/perfil/me'), { credentials: 'include' })
    .then(r => r.json())
    .then(data => {
  ...
  async function verificarPartidaAntesDeCrear() {
    ...
    try {
      ...
      const response = await fetch(apiUrl('/api/tablero/obtenerPartidasEnProgreso'), {
        credentials: 'include',
  ...
  window.eliminarPartida = function (partidaId) {
    if (confirm('¿Eliminar esta partida permanentemente?')) {
      ...
      fetch(apiUrl('/api/tablero/eliminarPartida'), {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  window.crearPartida = async function (esInvitado, nombreJugador2) {
    try {
      const userResponse = await fetch(apiUrl('/perfil/me'), { credentials: 'include' });
      ...
      const res = await fetch(apiUrl('/api/crearPartida'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          esInvitado,
  ...
  on('save-nickname-btn', 'click', async function() {
    ...
    try {
      const response = await fetch(apiUrl('/perfil/nickname'), {
        method: 'POST',
        headers: {
  ...
  async function cargarRankingData() {
    try {
      ...
      const userRankingResponse = await fetch(apiUrl('/perfil/user-ranking'), { credentials: 'include' });
      ...
      const rankingResponse = await fetch(apiUrl('/perfil/ranking?limit=3'), { credentials: 'include' });
